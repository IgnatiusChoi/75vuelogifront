...
export default {
  ...
  data() {
    return {
      itemCode: this.itemTable.itemCode,
      itemName: this.itemTable.itemName,
      itemGroupCode: this.itemTable.itemGroupCode,
      itemClassification: this.itemTable.itemClassification,
      unitOfStock: this.itemTable.unitOfStock,
      LossRate: this.itemTable.LossRate,
      leadTime: this.itemTable.leadTime,
      standardUnitPrice: this.itemTable.standardUnitPrice,
  ...
  methods: {
    ...
    inputModal() {
      ...
      console.log(data)
      this.closeModal()
      this.$store.dispatch('logi/base/UPDATE_ITEM',data)
          .then(this.$swal.fire(
              '수정 완료!',
              '품목 수정이 완료되었습니다.',
              'success',
